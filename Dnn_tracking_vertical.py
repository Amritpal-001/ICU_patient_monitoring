...
from track_utlis.centroidtracker import CentroidTracker
from track_utlis.trackableobject import TrackableObject
...
ct = CentroidTracker(maxDisappeared=40, maxDistance= 150)
trackers = []
trackableObjects = {}
...
while True:
	# grab the next frame and handle if we are reading from  VideoStream
	ret , frame = vs.read()
	if ret:
	...
	# object *detectors* to obtain a higher frame processing throughput
		...
		for tracker in trackers:
			...
			rects.append((startX, startY, endX, endY))
			#cv2.rectangle(frame ,startX , endY)

	# draw a horizontal line in the center of the frame -- once an
	# object crosses this line we will determine whether they were
	# moving 'up' or 'down'
	#cv2.line(frame, (0, H // 2), (W, H // 2), (0, 255, 255), 2)
	cv2.line(frame, (7 * H //4  , 0), ( 7 * H //4 , W), (0, 255, 255), 2)
	...
	for (objectID, centroid) in objects.items():
		...
		# to determine direction
			...
			if not to.counted:
				...
				if direction < 0 and centroid[0] < 7 * H //4:
				...
				elif direction > 0 and centroid[0] > 7 * H //4:
					print(centroid[0], centroid[1])
					totalDown += 1
					to.counted = True
